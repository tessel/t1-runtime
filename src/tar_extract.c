...
/*
** tm_mkdirhier() - create all directories in a given path
** returns:
**  0     success
...
tar_extract_regfile(TAR *t, char *realname)
{
  ...
  if (tm_mkdirhier(dirname(filename)) == -1)
    return -1;
...
#endif
  fd_res = tm_fs_open(&fdout, filename, TM_WRONLY | TM_CREATE_ALWAYS
#ifdef O_BINARY
//TODO    | O_BINARY
#endif
          );
//TODO     , 0666);
  ...
  for (i = size; i > 0; i -= T_BLOCKSIZE)
  {
    ...
    if (tm_fs_write(&fdout, buf,
        ((i > T_BLOCKSIZE) ? T_BLOCKSIZE : i), &nwritten) != 0)
      return -1;
  }

  /* close output file */
  if (tm_fs_close(&fdout) != 0)
    return -1;
...
tar_extract_dir(TAR *t, char *realname)
{
  ...
  int res = tm_fs_dir_create(filename);
  if (res != 0)
  {
//     if (errno == EEXIST)
//     {
