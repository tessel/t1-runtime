...
(self)["error"] = (null);
(self).finished = (false);
(self).autoFields = (not _truthy((not _truthy((options).autoFields))));
(self).autoFiles = (not _truthy((not _truthy((options).autoFields))));
...
if assert:ok((not _truthy((self)["error"])), err.stack) then end;
(self)["error"] = err;
...
if _truthy((not _truthy((self).flushing)) and (self).finished and (not _truthy((self)["error"]))) then
...
if (not _truthy(contentType)) then
...
if (index == (0)) then
i = i + boundaryEnd;
while _truthy((i < bufferLength) and (not _truthy(_in((buffer)[i], boundaryChars)))) do
...
(this).backpressure = (not _truthy((this).destStream:write(b:toString(("ascii")), ("base64"))));
else
(this).backpressure = (not _truthy((this).destStream:write(b)));
end
end)
