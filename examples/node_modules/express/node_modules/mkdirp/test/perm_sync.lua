function (_ENV)
local string, math, print, type, pairs = nil, nil, nil, nil, nil;
local _module = _obj({exports=_obj({})}); local exports, module = _module.exports, _module;

local mkdirp, path, fs, test = mkdirp, path, fs, test;
mkdirp = require(global, ("../"));
path = require(global, ("path"));
fs = require(global, ("fs"));
test = (require(global, ("tap"))).test;
if test(global, ("sync perm"), (function (this, t)
local file = file;
if t:plan((2)) then end;
file = ((("/tmp/") + (Math:random() * (_bit.lshift((1), (30)))):toString((16))) + (".json"));
if mkdirp:sync(file, (493)) then end;
if path:exists(file, (function (this, ex)
if (not (ex)) then
if t:fail(("file not created"))
         then end;
else
if fs:stat(file, (function (this, err, stat)
if _truthy(err) then
if t:fail(err)
             then end;
else
if t:equal(_bit.band((stat).mode, (511)), (493)) then end;
if t:ok(stat:isDirectory(), ("target not a directory")) then end;
if (function () local base, prop = t, "end"; return base[prop](base); end)() then end;
end
end))
     then end;
end
end)) then end;
end)) then end;
if test(global, ("sync root perm"), (function (this, t)
local file = file;
if t:plan((1)) then end;
file = ("/tmp");
if mkdirp:sync(file, (493)) then end;
if path:exists(file, (function (this, ex)
if (not (ex)) then
if t:fail(("file not created"))
         then end;
else
if fs:stat(file, (function (this, err, stat)
if _truthy(err) then
if t:fail(err)
             then end;
else
if t:ok(stat:isDirectory(), ("target not a directory")) then end;
if (function () local base, prop = t, "end"; return base[prop](base); end)() then end;
end
end))
     then end;
end
end)) then end;
end)) then end;

return _module.exports;
end